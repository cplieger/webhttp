...
v2 is one theme: a parameter you can get wrong silently becomes a parameter the compiler or the library checks. No behaviour changes beyond the HSTS guard below, and no symbol was removed.

Update the import path to `github.com/cplieger/webhttp/v2` (the `sse` subpackage moves with it to `github.com/cplieger/webhttp/v2/sse`; it is part of the same module), then apply five call-site changes:

| v1 | v2 |
| --- | --- |
| `WriteError(w, r, status, code, msg string)` | `WriteError(w, r, status, code ErrorCode, msg string)` — wrap a bare literal or a `string` variable as `webhttp.ErrorCode(code)`; an app whose codes are already a defined string type (`type ErrorCode string`, which several consumers have) can drop its `string(code)` conversion and pass the value through |
| `WithHostAllowlistError(code, msg string)` | `WithHostAllowlistError(code ErrorCode, msg string)` — same conversion |
| `WithRateLimitError(code, msg string)` | `WithRateLimitError(code ErrorCode, msg string)` — same conversion |
| `WithHSTS(maxAge, includeSubdomains, preload bool)` | `WithHSTS(HSTS{MaxAge: …, IncludeSubdomains: …, Preload: …})` |
| `WithLoopbackExempt()` | `WithLoopbackExempt(true)` |
| `WithSkipUpgrades()` | `WithSkipUpgrades(true)` |

Also typed by the first three rows: `ErrorResponse.Code` is now an `ErrorCode` (decoding is unchanged, and comparing it to a string literal still compiles), and `ErrorResponder`'s fourth parameter is an `ErrorCode` — a custom responder's signature changes, and a `string(code)` conversion is needed where it concatenates the code into a body.

**Why the error code is a type.** The code is a machine token and the message is a sentence for a human, and as two adjacent `string` parameters they could be handed over transposed — putting the sentence in the slot a client switches on and an alert rule keys on, with nothing to catch it. Typed apart, that transposition is a compile error wherever either side is a named constant or a variable, which is every call site in this fleet that does not inline both strings. For the residue — two literals typed in the wrong order, which no string-based type can reject, because an untyped constant converts — the envelope encoder is the second gate: a code that breaks the grammar (anything outside `[a-z0-9_]`, a space above all) is replaced with `invalid_error_code`, and the offender is logged once per process. It does not panic: `WriteError` runs per request on an error path, so a panic there would turn a handled 4xx into a dropped connection, or into a 500 that blames the server for the client's mistake.

**Why HSTS is a struct, and the two guards it gained.** Its two booleans stood for different directives, so a transposition built a different-but-valid header; as named fields they cannot be confused. A policy asking for `Preload` while failing a submission criterion is now REFUSED rather than emitted: the browsers' preload list rejects a submission without `includeSubDomains` OR with a `max-age` under one year ([hstspreload.org](https://hstspreload.org)), so the header advertised a posture the operator did not actually get, and the failure surfaced only at submission time. Both criteria are policed for that one reason; without `Preload` the max-age is unconstrained, because a short max-age is a valid working policy that simply cannot be preloaded. The directive is dropped and a warning names the field to set. A `SecurityOption` cannot return an error, so that is the lenient half of the drop-and-report shape `ParseCIDRs` and `ParseHostList` already use here; `HSTS.Validate() error` is the strict half, for a caller that builds its policy from configuration and would rather refuse to boot. The two rejected alternatives: panicking (wrong for a value that can come from config — `Validate` is the door instead), and silently setting `IncludeSubdomains` (that would widen a security commitment the caller never asked for).

**Why the two flag options take a parameter.** An option that only ever says "true" cannot express the other half of its own decision, so a caller with a config field or an env var has to branch around the option instead of passing its value. `false` means exactly what leaving the option out means, options resolve last-wins in both directions, and a `HostPolicy` built from unset configuration still behaves identically.
