...
- `RateLimiter(burst int, interval time.Duration, opts...) Middleware` — throttles the wrapped handler through a single **process-wide** token bucket (standard library only): `burst` tokens (the max and the initial fill), one accrued every `interval`, one consumed per admitted request; a request that finds the bucket empty gets a 429 via `WriteError` (`code: "rate_limited"`) and does not reach the handler. The 429 carries a `Retry-After` hint of the whole seconds for the current token deficit to refill (at most `ceil(interval)` on a freshly emptied bucket, less once partially refilled, at least 1s). The bucket is shared across all clients, so it bounds the **aggregate** rate of an expensive shared route (for example spawning a heavy child process per request), not per-client fairness. Options: `WithRateLimitWhen(pred)` limits only requests the predicate matches (throttle a single method+path on a handler serving several), `WithRateLimitError(code, msg)` overrides the 429 code and message. A **non-positive** `burst` or `interval` disables limiting (returns the handler unwrapped), so a config-driven zero cleanly means "no limit" (mirroring `RouteTimeout`'s off contract)
...
- `ParseHostList(entries []string, opts...) (*HostPolicy, invalid []string)` — parse an operator-supplied allowlist (a config array or a comma-split env var) into an immutable exact-match `HostPolicy`; malformed entries are returned separately (the `ParseCIDRs` shape) so a strict caller can refuse startup while a lenient one logs and proceeds
- `CanonicalHost(hostport string) string` — the strict authority canonicalizer both the entries and each request's `Host` go through: lowercase, no port, no brackets, at most one trailing FQDN dot, IP literals rendered through `net.ParseIP` so different spellings of one address compare equal (`[::ffff:192.0.2.1]` equals `192.0.2.1`); returns `""` for anything malformed
- `(*HostPolicy).Middleware() Middleware` — rejects a request whose `Host` is not allowlisted with a 403 JSON `ErrorResponse` (`code: "host_not_allowed"`); an **inactive** policy returns the handler unwrapped (the `RateLimiter`/`RouteTimeout` off contract)
- `(*HostPolicy).Allows(r)`, `.Active()`, `.Size()` — the per-request decision and policy introspection
- `WithLoopbackExempt() HostAllowlistOption` — admit a request when **both** the socket peer and the `Host` are loopback, so a baked Docker healthcheck or in-container client keeps working under a browser-facing allowlist; unreachable by rebinding (the attack's `Host` is not loopback) and by remote forgery (a remote peer is not loopback)
...
The gate exists to break **DNS rebinding** (CWE-346): an attacker's page re-resolves its own hostname to this service's address, and the victim's browser then sends same-origin requests that reach the service carrying the attacker's name in `Host`. Matching is **exact and purely textual** on the canonicalized `Host` — no name resolution (resolving would reopen the very race the gate closes), and `X-Forwarded-Host` is ignored (client-controlled). Malformed `Host` values are **rejected, never repaired**: deleting stray syntax would let distinct wire values (`[allowed.example]`, `allowed.example:garbage:443`) collapse onto an allowlisted key. Non-ASCII names must be configured as Punycode A-labels (matching is byte-exact; no IDN mapping), and IPv4-like strings `net.ParseIP` rejects (`127.0.0.001`) are reported invalid rather than kept as silently unmatchable names.

Activation is fail-closed: a nil or all-blank entry list leaves the policy **inactive** (pass-through, the backward-compatible default for unset configuration), but any non-blank entry engages the gate — even when every entry is invalid, in which case the active empty allowlist **denies all** rather than silently disabling protection. Place the middleware outermost among request-authorization middleware, before any cross-origin or CSRF check: a rebinding request makes `Origin` and `Host` agree, so the exact-Host allowlist is what breaks that chain.
...
- Options: `WithLogger(l)`, `WithSkipPaths(paths...)`, `WithSkipFunc(fn)`, `WithRecordMetric(fn)`, `WithRecordMetricRequest(fn)`, `WithLogLevel(fn)`, `WithClientIP(trusted...)`, `WithClientIPFunc(fn)`
...
- `WithLogLevel(fn func(r *http.Request, status int) slog.Level)` sets the access-line **level policy** (default: every line at `Info`). The canonical use is scrape-noise control on a polled service: 2xx/3xx at `Debug` so a 15-second Prometheus scrape stays out of the log stream at the default level, `Warn` for 4xx, `Error` for 5xx. The hook picks the level only; attributes and emission rules stay fixed. Skipped paths never call it (no line at all); a panicking policy is contained and the line falls back to `Info`; nil is ignored.
...
The verification primitive for static machine credentials — an API key, a bearer token, a basic-auth user or password — where exactly one expected value comes from configuration. The configured secret is hashed with SHA-256 once at construction; `Verify` hashes only the presented value and compares the two fixed-length digests with `subtle.ConstantTimeCompare`. Comparing 32-byte digests rather than the raw strings removes `ConstantTimeCompare`'s unequal-length short-circuit (CWE-208), and pre-hashing the configured secret means no per-call timing varies with the secret's length or content — hashing the presented value per call reveals only what the caller already knows: its own input.

An empty `configured` secret **fails closed**: `Verify` returns `false` for every presented value, including the empty string (the zero-value verifier behaves the same). Without that guard the digest comparison would fail _open_ — `sha256("")` equals `sha256("")`, so an unset secret would grant access to any client that simply presents no credential. Treat an empty configured value as "auth not configured", never as "no credential required"; a caller that wants an open endpoint should skip the gate explicitly. This verifies one shared secret, not user identities: per-user credential stores, password hashing, and session management belong to the [auth](https://github.com/cplieger/auth) library.
